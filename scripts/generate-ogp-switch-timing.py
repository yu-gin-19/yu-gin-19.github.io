"""
OGP画像生成: articles/switch-timing/ogp.png 専用スクリプト

scripts/generate-ogp-batch.py はCONFIGSが常に25件であることを前提に検証しており、
記事ごとの3D素材（ChatGPT側で事前作成）がないと生成できない。switch-timing記事には
まだ専用の3D素材がないため、このスクリプトでは既存のA案背景・フォント探索ロジックのみを
再利用し、3D素材の合成なしで1200x630pxのOGP画像を1枚だけ生成する。

使い方:
    python scripts/generate-ogp-switch-timing.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "switch-timing", "ogp.png")
...
def render():
    ...
    # 大見出し
    main = [("月末が正解", "white"), ("？", "magenta")]
    f_big = fit_font_segments(draw, main, A_TEXT_MAX_WIDTH, 148, 72)
    draw_segments(draw, 66, 450, main, f_big)

    # 黄色帯に濃紺文字
    band = "申し込み日・開通日・料金を整理"
...
def main():
    ...
    print(f"[{ok}] articles/switch-timing/ogp.png ({size[0]}x{size[1]})")
