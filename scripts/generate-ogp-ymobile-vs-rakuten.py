"""
OGP画像生成: articles/ymobile-vs-rakuten/ogp.png 専用スクリプト
...
（scripts/generate-ogp-uq-vs-rakuten.py, generate-ogp-linemo-vs-rakuten.py と同じ方針）。

このOGPには料金・ポイント数字や「必ず安くなる」「乗り換えるべき」等、記事の
結論を先取りする訴求文言を一切載せない。メインキャッチは「Y!mobile×楽天モバイル」、
黄色帯のサブコピーで「家族割・おうち割、どう変わる？」という中立的な問いかけを示す。

使い方:
    python scripts/generate-ogp-ymobile-vs-rakuten.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "ymobile-vs-rakuten", "ogp.png")
...
COLOR_PURPLE = (200, 150, 245)
COLOR_NAVY_TEXT = (18, 24, 58)
COLOR_NOTE = (210, 214, 226)
COLOR_NOTE_STROKE = (10, 10, 14)

COLOR_ROLES = {"white": COLOR_WHITE, "magenta": COLOR_MAGENTA, "yellow": COLOR_YELLOW, "purple": COLOR_PURPLE}
...
def render():
    ...
    # 大見出し（結論を先取りする数字・断定表現は使わず、比較対象名のみ）
    main = [("Y!mobile", "purple"), ("×楽天モバイル", "white")]
    f_big = fit_font_segments(draw, main, A_TEXT_MAX_WIDTH, 64, 30)
    draw_segments(draw, 66, 430, main, f_big)

    # 黄色帯に濃紺文字（サブコピー。中立的な問いかけに留める）
    band = "家族割・おうち割、どう変わる？"
    f_band = fit_font(draw, band, 900, 46, 24)
...
def main():
    ...
    print(f"[{ok}] articles/ymobile-vs-rakuten/ogp.png ({size[0]}x{size[1]})")
