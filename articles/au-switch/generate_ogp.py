"""
OGP画像生成スクリプト（au-switch記事用）
既存記事（card-review / demerits / fit-check / savings-calc）のOGPパターンを踏襲：
- 背景：左ピンク→右ブラックの斜め方向グラデーション＋ダイヤ格子の装飾線
- 右側：キャラクター（sns/images/character.png）を配置
- 左側：サイト名ラベル・見出し2行・カテゴリバッジ
- 右下：「個人運営・非公式」の注記バッジ

出力: OGP_au-switch.png（1200×630px）
"""
...
OUT_PATH = os.path.join(BASE_DIR, "OGP_au-switch.png")
...
def main():
    ...
    draw.text((margin, 275), line2, font=f_title, fill=COLOR_WHITE)

    # サブコピー
    f_sub = ImageFont.truetype(FONT_MEDIUM, 26)
    draw.text((margin, 350), "5年累計26万円以上の節約を社員が公開", font=f_sub, fill=COLOR_WHITE_SUB)
