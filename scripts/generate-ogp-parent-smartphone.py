"""
OGP画像生成: articles/parent-smartphone/ogp.png 専用スクリプト

scripts/generate-ogp-batch.py はCONFIGSが常に25件であることを前提に検証しており、
記事ごとの3D素材（ChatGPT側で事前作成）がないと生成できない。parent-smartphone記事には
専用の3D素材がないため、既存のA案背景・フォント探索ロジックのみを再利用し、
3D素材の合成なしで1200x630pxのOGP画像を1枚だけ生成する
（scripts/generate-ogp-kids-smartphone.py と同じ方針）。

「月858円」などの金額だけを大きく見せると、実質額の条件（ポイント還元・家族割適用時
であること）が画像内で説明しきれず誤解を招くため、このOGPでは金額を大きく訴求せず、
「実質額だけで決めない」という注意喚起を見出しにする。

使い方:
    python scripts/generate-ogp-parent-smartphone.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "parent-smartphone", "ogp.png")
...
def render():
    ...
    # 大見出し（金額を大きく見せず、判断の軸を訴求する）
    main = [("実質額", "white"), ("だけで決めない", "magenta")]
...
def main():
    ...
    print(f"[{ok}] articles/parent-smartphone/ogp.png ({size[0]}x{size[1]})")
