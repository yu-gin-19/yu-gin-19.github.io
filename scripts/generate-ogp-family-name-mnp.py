"""
OGP画像生成: articles/family-name-mnp/ogp.png 専用スクリプト

scripts/generate-ogp-batch.py はCONFIGSが常に25件であることを前提に検証しており、
記事ごとの3D素材（ChatGPT側で事前作成）がないと生成できない。family-name-mnp記事には
専用の3D素材がないため、既存のA案背景・フォント探索ロジックのみを再利用し、
3D素材の合成なしで1200x630pxのOGP画像を1枚だけ生成する
（scripts/generate-ogp-switch-timing.py と同じ方針）。

実績・数字は創作せず、文字中心のチェック・注意型のみで構成する。

使い方:
    python scripts/generate-ogp-family-name-mnp.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "family-name-mnp", "ogp.png")
...
def render():
    ...
    # 中見出し
    mid = "スマホの契約が"
    f_mid = fit_font(draw, mid, A_TEXT_MAX_WIDTH, 52, 30)
    draw.text((72, 300), mid, font=f_mid, fill=COLOR_WHITE)

    # 大見出し
    main = [("家族名義のまま", "white"), ("？", "magenta")]
    f_big = fit_font_segments(draw, main, A_TEXT_MAX_WIDTH, 120, 60)
    draw_segments(draw, 66, 450, main, f_big)

    # 黄色帯に濃紺文字
    band = "乗り換え前に名義を確認"
    f_band = fit_font(draw, band, 900, 58, 30)
...
def main():
    ...
    print(f"[{ok}] articles/family-name-mnp/ogp.png ({size[0]}x{size[1]})")
