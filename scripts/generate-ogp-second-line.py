"""
OGP画像生成: articles/second-line/ogp.png 専用スクリプト

scripts/generate-ogp-batch.py はCONFIGSが常に25件であることを前提に検証しており、
記事ごとの3D素材（ChatGPT側で事前作成）がないと生成できない。second-line記事には
専用の3D素材がないため、既存のA案背景・フォント探索ロジックのみを再利用し、
3D素材の合成なしで1200x630pxのOGP画像を1枚だけ生成する
（scripts/generate-ogp-parent-smartphone.py と同じ方針）。

デザイン方針（projects/second-line/design/design-direction.md 6節）により、
このOGPには料金・ポイント数字を一切載せない。メインキャッチは「今の番号はそのまま」、
黄色帯のサブコピーで「楽天を2回線目に」を訴求する。

使い方:
    python scripts/generate-ogp-second-line.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "second-line", "ogp.png")
...
def render():
    ...
    # 大見出し（価格・ポイント数字は使わず、価値訴求のコピーのみ）
    main = [("今の番号は", "white"), ("そのまま", "magenta")]
...
def main():
    ...
    print(f"[{ok}] articles/second-line/ogp.png ({size[0]}x{size[1]})")
