"""
OGP画像生成: articles/kids-smartphone/ogp.png 専用スクリプト

scripts/generate-ogp-batch.py はCONFIGSが常に25件であることを前提に検証しており、
記事ごとの3D素材（ChatGPT側で事前作成）がないと生成できない。kids-smartphone記事には
専用の3D素材がないため、既存のA案背景・フォント探索ロジックのみを再利用し、
3D素材の合成なしで1200x630pxのOGP画像を1枚だけ生成する
（scripts/generate-ogp-overseas-roaming.py と同じ方針）。

実績・数字は創作せず、記事本文と一致する「実質1,078円」の表記のみを使う。

使い方:
    python scripts/generate-ogp-kids-smartphone.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "kids-smartphone", "ogp.png")
...
def main():
    ...
    print(f"[{ok}] articles/kids-smartphone/ogp.png ({size[0]}x{size[1]})")
