"""
OGP画像生成: articles/current-phone-compatibility/ogp.png 専用スクリプト

scripts/generate-ogp-batch.py はCONFIGSが常に25件であることを前提に検証しており、
記事ごとの3D素材（ChatGPT側で事前作成）がないと生成できない。current-phone-compatibility記事には
専用の3D素材がないため、既存のA案背景・フォント探索ロジックのみを再利用し、
3D素材の合成なしで1200x630pxのOGP画像を1枚だけ生成する
（scripts/generate-ogp-device-balance-mnp.py と同じ方針）。

実績・数字は創作せず、文字中心のチェック・注意型のみで構成する。

使い方:
    python scripts/generate-ogp-current-phone-compatibility.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "current-phone-compatibility", "ogp.png")
...
def render():
    ...
    # 大見出し
    main = [("楽天モバイルで", "white"), ("使える？", "magenta")]
    f_big = fit_font_segments(draw, main, A_TEXT_MAX_WIDTH, 100, 48)
...
def main():
    ...
    print(f"[{ok}] articles/current-phone-compatibility/ogp.png ({size[0]}x{size[1]})")
