"""
OGP画像生成: articles/device-balance-mnp/ogp.png 専用スクリプト

scripts/generate-ogp-batch.py はCONFIGSが常に25件であることを前提に検証しており、
記事ごとの3D素材（ChatGPT側で事前作成）がないと生成できない。device-balance-mnp記事には
専用の3D素材がないため、既存のA案背景・フォント探索ロジックのみを再利用し、
3D素材の合成なしで1200x630pxのOGP画像を1枚だけ生成する
（scripts/generate-ogp-family-name-mnp.py と同じ方針）。

実績・数字は創作せず、文字中心のチェック・注意型のみで構成する。

使い方:
    python scripts/generate-ogp-device-balance-mnp.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "device-balance-mnp", "ogp.png")
...
def render():
    ...
    # 大見出し
    main = [("楽天モバイルへ", "white"), ("乗り換えできる", "magenta")]
    f_big = fit_font_segments(draw, main, A_TEXT_MAX_WIDTH, 92, 44)
...
def main():
    ...
    print(f"[{ok}] articles/device-balance-mnp/ogp.png ({size[0]}x{size[1]})")
