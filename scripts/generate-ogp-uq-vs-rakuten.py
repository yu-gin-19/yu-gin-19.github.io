"""
OGP画像生成: articles/uq-vs-rakuten/ogp.png 専用スクリプト
...
（scripts/generate-ogp-povo-rakuten-dual-sim.py と同じ方針）。

このOGPには料金・ポイント数字や「楽天が絶対安い」「乗り換えるべき」等、記事の
結論を先取りする訴求文言を一切載せない。メインキャッチは「UQ mobile×楽天モバイル」、
黄色帯のサブコピーで「自宅セット割なしでも続ける？」という中立的な問いかけを示す。

使い方:
    python scripts/generate-ogp-uq-vs-rakuten.py
"""
...
OUT_PATH = os.path.join(REPO_ROOT, "articles", "uq-vs-rakuten", "ogp.png")
...
COLOR_TEAL = (0, 200, 220)
COLOR_NAVY_TEXT = (18, 24, 58)
COLOR_NOTE = (210, 214, 226)
COLOR_NOTE_STROKE = (10, 10, 14)

COLOR_ROLES = {"white": COLOR_WHITE, "magenta": COLOR_MAGENTA, "yellow": COLOR_YELLOW, "teal": COLOR_TEAL}
...
def render():
    ...
    main = [("UQ mobile", "teal"), ("×楽天モバイル", "white")]
...
def main():
    ...
    print(f"[{ok}] articles/uq-vs-rakuten/ogp.png ({size[0]}x{size[1]})")
